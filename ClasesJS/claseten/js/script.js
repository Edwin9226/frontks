...
for (var i = 0; i < drawNumber.length; i++) {
  context.beginPath();
  context.moveTo(center, center);
  context.arc(
    center,
    center,
    center - 20,
    (i * 2 * Math.PI) / drawNumber.length,
    ((i + 1) * 2 * Math.PI) / drawNumber.length
  );
  context.lineTo(center, center);
  ...
  context.rotate(
    (3 * 2 * Math.PI) / (5 * drawNumber.length) +
      (i * 2 * Math.PI) / drawNumber.length
  );
  context.translate(-center, -center);
  ...
  context.fillText(drawNumber[i], canvas.width - 30, center);
  context.restore();
}
...
function sortear() {
  if (clic == 0) {
    let canvas = document.getElementById("idcanvas");
    console.log(canvas.COMMENT_NODE);
